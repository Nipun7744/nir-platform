...
| GET | `/innovations/admin/repository` | **Admin Repository Management** listing (2026-08-17) — innovations with `reviewStatus` in `PUBLISHED`/`UNPUBLISHED` only (`RepositoryFilterDto`: `status`, `categoryId`, `q` — matches `titleEn`/`titleBn`/`innovationCode` — `fromDate`/`toDate` against `publishedAt`, `page`, `pageSize`), ordered by `publishedAt desc`. Backs `/dashboard/admin/repository`'s publish/unpublish + media management UI | `PLATFORM_ADMIN, SYSTEM_ADMIN` |
